...
| セットの入力(重量・回数は Digital Crown、RIR 0/1/2/3+) | 記録の保存・破棄 |
...
Watch で Crown を回していない値は、iPhone で入力されたままの文字列を送り返す。`Double()` で読めない `"80,5"` などを 0 で上書きしないため。
...
- 実機の Watch をタップして操作したときの使い心地(Digital Crown の感度、RIR ボタンの押しやすさ)
