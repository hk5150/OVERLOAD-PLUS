...
- [ ] 休憩タイマーの経過(1・2・3分)を**ローカル通知**で知らせる(v96で追加)。
      すべて端末内で完結し、プッシュ通知サーバは使わない
- [ ] 許可はランタイムで要求するため `Info.plist` へのキー追加は不要
- [ ] `@capacitor/local-notifications@6.1.3` は Required Reason API を使っておらず、
      自前の `.xcprivacy` も同梱していないため `PrivacyInfo.xcprivacy` の追加宣言は不要
      (プラグインの `ios/Sources/` を実際に検索して確認済み)
- [ ] App Privacy の回答は「データを収集しない」のまま変わらない
      (通知の内容は端末内で生成され、外部へ送信されない)
- [ ] 許可を拒否された場合は従来の通知音にフォールバックし、設定タブに
      iOSの設定アプリへの案内を表示する(拒否後はアプリから再要求できないため)
