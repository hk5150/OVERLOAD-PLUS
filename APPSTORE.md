...
### 1.0 (ビルド3) — 2026-09-26 0:18 に審査へ提出

App Store Connect の Apple ID は 6816090624、SKU は `kurabell-ios-001`。アプリ内課金 `Full Unlock` と同時に提出した。

入力のときに決めたこと(この文書の上の方針に書いていなかったもの):
- **価格と配信**: 無料、175か国。**Apple シリコン搭載の Mac と Apple Vision Pro での配信はオフ**
  (未検証の環境で、Mac にはヘルスケアも無い。審査で Mac の動作を見られるのも避けた)
- **年齢制限**: すべて「なし/いいえ」で 4+。「健康またはウェルネス関連(セルフケアや生活習慣の推奨)」も「いいえ」
  (記録を並べて見せるだけで、生活習慣の推奨はしない)
- **コンテンツ配信権**: 第三者のコンテンツは含まない(YouTube・AIチャットは外部ブラウザで開くだけ)
- **規制対象の医療用デバイス**: 該当しないと申告(ヘルスケア/フィットネスのカテゴリでは申告が必須)
- **プライバシー**: 「データの収集なし」を公開。ポリシー URL は日英とも同じ `privacy.html`
- **リリース**: 承認後に自動でリリース
- **著作権**: `© 2026 Hajime Kitamura`
