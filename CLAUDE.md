...
- **バージョンは2箇所ある。** `sw.js` の `CACHE` と、ヘッダーのバッジ。
  ...
  **v114 からアプリ本体はキャッシュ優先**(`sw.js`)。版ごとのファイル一式は install で全部取れたときだけ
  入れ替わるので、**`CACHE` を上げない限り本番の利用者に新しいファイルは届かない**。
  新しい版が有効になるとページに「新しいバージョンがあります」のバナーが出る。起動に失敗している画面は
  自動で1回だけ再読み込みする(index.html末尾のSW登録ブロック)。それ以前に新旧のファイルが混ざっていた
  経緯(HTTPキャッシュ、電波が弱いときのファイルごとのタイムアウト)は `docs/vite移行.md` のv113・v114節。
  **アプリとして開くのは `./` と `./index.html` だけ。** 同じスコープの `privacy.html`・`support.html`
  (App Store Connectに登録しているURL)まで index.html で返すと、Web版を開いたことのあるブラウザで
  読めなくなる(v114のレビューで見つかった)。HTMLページを足すときはテストで縛ってある経路を確認すること。
  **localhost では既定で SW を登録しない**(キャッシュ優先だと編集しても再読み込みで古い版が出続けるため)。
  SW自体を検証するときだけ `?sw=1` を付けて開く。付けずに開くと登録済みのSWは解除されるが、その回の表示は
  まだ保存済みの古い版で、新しいコードが出るのは次の読み込みから。
  **ただし localhost(SWなし)でも、HTTPキャッシュ単独で古いスクリプトが出ることはある。**
